-- Una fuente para todo el perfil. Los clientes anteriores conservan el valor al guardar.
alter table public.profiles add column fuente text
  check (fuente is null or fuente in ('revista','redonda','maquina','manuscrita','cartel','retro'));
...
as $$
  select p.user_id, p.username, p.display_name, p.avatar_path, p.bio,
         ...
         p.avatar_encuadre, p.banner_encuadre, p.marco, p.tema, p.fuente
...
drop function if exists public.update_my_profile(text, text, text, text, text, text, jsonb, jsonb, text, jsonb);
...
begin
  ...
  return query
    select p.user_id, p.username, p.display_name, p.avatar_path, p.bio, p.banner_path,
           p.created_at, p.visibility, p.avatar_encuadre, p.banner_encuadre, p.marco, p.tema, p.fuente
    from public.profiles p
    where p.user_id = me;
end;
$$;
...
revoke all on function public.update_my_profile(text, text, text, text, text, text, jsonb, jsonb, text, jsonb, text) from public;
grant execute on function public.get_my_profile() to authenticated;
grant execute on function public.get_profile(text) to authenticated;
grant execute on function public.update_my_profile(text, text, text, text, text, text, jsonb, jsonb, text, jsonb, text) to authenticated;
