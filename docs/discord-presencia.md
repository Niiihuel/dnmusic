...
La posición se toma del motor que realmente reproduce. Un dispositivo espejo
no puede escribir su `currentTime` local al store, ni desde eventos de audio,
ni al volver al frente, ni desde un callback anterior al cambio de salida.
Al despertar, el espejo vuelve a derivar el segundo actual usando el reloj del
servidor antes de pedir el estado fresco; al cambiar de tema aplica canción y
posición juntas sin esperar la lectura completa de la cola.

El payload externo separa `sampledAt` (instante de la posición) de `updatedAt`
(frescura del último latido del dispositivo emisor). El puente compensa la edad
de la muestra y conserva el inicio absoluto de la canción; no vuelve a sumar
la edad de un latido a una posición remota ya extrapolada. Los clientes antiguos
sin `sampledAt` siguen siendo aceptados. Se usa la duración medida por el motor
cuando está disponible.

Las actualizaciones de actividad se agrupan cada cinco segundos como máximo
(fuera de esperas por ACK o reconexiones), con borrado inmediato en pausa. La
reconexión conserva su espera de quince segundos. No se inicia audio en otro
dispositivo para mantener su indicador al día.

Regresiones: `tests/motor-audio-integracion.test.mjs`,
`tests/escucha-transferencia.test.mjs`, `tests/actividad-escucha.test.mjs` y
`desktop/tests/discord-presence.test.cjs` cubren despertar, eventos tardíos,
pausas, cambios de tema y muestras demoradas. La comprobación física PC/teléfono
requiere que ambos ejecuten una compilación con estos cambios.
