...
export function objetivoResizeMosaico(inicio: { w: number; h: number; mosaico: number; filas: number }, dx: number, dy: number) {
  'worklet'
  ...
  const cols = w > columna * 1.5 ? 2 : 1
  return { cols, filas: cols === 2 && h > filaBase * 1.5 ? 2 : 1 }
}
