...
export function AccionSocial({ label, onPress, secundaria = false, busy = false, disabled = false, icono, expandida, style }: {
  label: string; onPress: () => void; secundaria?: boolean; busy?: boolean; disabled?: boolean; icono?: ReactNode; expandida?: boolean; style?: StyleProp<ViewStyle>
}) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled: inactiva, busy }}
    ...
    style={[{ alignSelf: (expandida ?? !escritorio) ? 'stretch' : 'flex-start', maxWidth: '100%' }, style]}
    className={`min-h-11 flex-row items-center justify-center gap-2 rounded-2xl px-4 py-3 ${secundaria || inactiva ? 'bg-muted' : 'bg-primary'} active:opacity-75`}>
    {busy ? <ActivityIndicator size="small" color={ICON_COLOR.foreground} /> : icono}
    <Text style={{ flexShrink: 1 }} className={`text-center text-[15px] font-semibold ${inactiva ? 'text-muted-foreground' : secundaria ? 'text-foreground' : 'text-primary-foreground'}`}>{label}</Text>
  </Pressable>
}
