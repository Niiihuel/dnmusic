...
export function CabeceraChats({ cantidad, techo, onNew }: CabeceraChatsProps) {
  ...
  return <View style={{ paddingTop: techo, paddingBottom: 8, flexShrink: 0 }}>
  <Host matchContents={{ vertical: true }} ignoreSafeArea="all" colorScheme="dark" seedColor="#FFFFFF" style={{ width: '100%', minHeight: 76, flexShrink: 0 }}>
    <HStack alignment="center" spacing={12} modifiers={[padding({ horizontal: 16, vertical: 8 }), frame({ minHeight: 76 })]}>
...
export function TituloSeccionChats({ texto }: { texto: string }) {
  return <Host matchContents={{ vertical: true }} ignoreSafeArea="all" colorScheme="dark" style={{ width: '100%' }}>
...
export function FilaConversacion({ nombre, nombreAvatar = nombre, avatarPath, detalle, fecha, noLeidos, selected, onPress }: FilaConversacionProps) {
  ...
  return <Host matchContents={{ vertical: true }} ignoreSafeArea="all" colorScheme="dark" seedColor="#FFFFFF" style={{ width: '100%', minHeight: 76 }}>
...
export function FilaSolicitudChat({ nombre, etiqueta, avatarPath, onAceptar, onRechazar }: FilaSolicitudChatProps) {
  return <Host matchContents={{ vertical: true }} ignoreSafeArea="all" colorScheme="dark" seedColor="#FFFFFF" style={{ width: '100%' }}>
