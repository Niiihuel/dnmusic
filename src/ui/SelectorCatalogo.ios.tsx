...
export function SelectorCatalogo({ etiqueta, valor, opciones, onChange }: Props) {
  ...
  if (opciones.length <= 8) return <Host matchContents colorScheme="dark" seedColor="#FFFFFF" style={{ minHeight: 44 }}>
  ...
  return <>
    <AccionSocial label={`${etiqueta}: ${texto}`} secundaria expandida={false} onPress={() => setAbierto(true)} />
    <Modal visible={abierto} animationType="slide" presentationStyle="pageSheet" onRequestClose={cerrar}>
      <View style={{ flex: 1, backgroundColor: '#121212' }}>
        ...
        <Host colorScheme="dark" seedColor="#FFFFFF" style={{ flex: 1 }} useViewportSizeMeasurement>
