...
import { ICON_COLOR, IconMore } from './icons'
...
export function Menu({
  ...
  const idealH = usable.length * ROW_H
  ...
  const openMenu = () => {
    if (Platform.OS === 'ios') {
      const destructivas = usable
        .map((item, i) => (item.destructive ? i : -1))
        .filter((i) => i >= 0)
      ActionSheetIOS.showActionSheetWithOptions(
        {
          options: [...usable.map((item) => item.label), 'Cancelar'],
    ...
    <>
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        ...
        <View className="flex-1">
          ...
          />
          <View
            style={{ position: 'absolute', top, left, width: MENU_W, maxHeight: menuH }}
