...
export function FilaSocial({ titulo, detalle, fontFamily, valor, label, selected = false, busy = false, disabled = false, onPress }: FilaSocialProps) {
  return <Pressable accessibilityRole="button" accessibilityLabel={label ?? [titulo, detalle, valor].filter(Boolean).join(', ')}
    ...
    {busy ? <ActivityIndicator color="#B3B3B3" /> : valor ? <Text className="text-muted-foreground text-footnote">{valor}</Text> : selected ? <Text className="text-foreground">✓</Text> : null}
  </Pressable>
}
