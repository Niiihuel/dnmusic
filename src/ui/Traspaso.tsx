...
export function Traspaso() {
  ...
  if (!pendiente) return <Dialogo titulo="Traer la música" visible={false} contenidoPC={null} />
  const contenido = (<>
  ...
  const contenidoPC = <><EncabezadoHoja titulo="Traer la música" izquierda={<BotonHoja onPress={cancelar} disabled={ocupado} />} /><View className="gap-3 px-5 pb-5">
    <Text className="text-muted-foreground text-subheadline leading-6">La escucha está en «{pendiente.nombre}». Podés traerla acá en el segundo por el que va, o dejarla donde está.</Text>
    {ocupado ? <Text accessibilityLiveRegion="polite" className="text-muted-foreground text-footnote">Esperando que se confirme el cambio de dispositivo.</Text> : null}
    {error ? <Text accessibilityRole="alert" accessibilityLiveRegion="polite" className="text-foreground text-footnote">{error}</Text> : null}
    <PrimaryButton label="Traer acá" onPress={() => { void confirmar() }} busy={ocupado} />
    <GhostButton label="Seguir allá" onPress={cancelar} disabled={ocupado} />
  </View></>
  return <Dialogo titulo="Traer la música" ancho={400} contenidoPC={contenidoPC} visible transparent animationType="fade" onRequestClose={cancelar} accessibilityLabel="Traer la música">
