  ...
  const fila = (
    ...
    >
      ...
      >
         ...
         */}
        <Text
          className="text-muted-foreground text-caption1 tabular-nums"
      ...
      {suelto ? null : (
        <View style={{ width: ANCHO_DURACION, height: 44, alignItems: 'center', justifyContent: 'center' }}>
          <Text pointerEvents="none" className="text-center text-muted-foreground text-caption1 tabular-nums"
...
/**
 * Lo que mide la columna de la duración, en la fila y en su encabezado.
 *
 * Es una sola constante y no dos números iguales escritos aparte porque los dos
 * **tienen** que coincidir o el rótulo se despega de su columna. Y son 56 y no
 * 48 porque «Duración» no entraba: se partía al medio en dos renglones, que ya
 * había pasado antes y volvió a pasar al ponerle a la escala su tracking. El
 * `numberOfLines={1}` del rótulo es el seguro para la próxima.
 *
 * Son 64 y no los 56 que salían de medir «Duración» al ras (55px): al ras el
 * navegador redondea y lo elipsa igual, y de paso la palabra mide distinto en
 * cada sistema —SF Pro, Segoe UI, el fallback de Linux—. Sobra ancho, que en
...
export function TrackColumnHeader({ trailing = 36 }: { trailing?: number }) {
  ...
  return (
    <View className="mx-6 mb-1 flex-row items-center gap-4 border-b border-muted px-2 pb-2">
      <Text className="w-6 text-center text-muted-foreground text-caption1">#</Text>
      {/* En oración normal y los tres del mismo tamaño, que es como titula sus
          columnas la tabla de Música en la Mac. Antes «Título» iba en
