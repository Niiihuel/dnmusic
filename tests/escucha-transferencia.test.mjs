...
function fixture({jam=false,playing=true,owner='pc',timestamp=true,send,claim,read}={}){
 let now=100000, seq=0, hooks, registered, fetches=0, foreground
...
test('el espejo entra en el segundo actual, avanza por reloj y se congela al pausar',async()=>{
 const h=fixture();h.remote({posicionMs:90000});await h.start()
 assert.equal(h.p.positionMs,90000)
 await h.advance(5000);assert.equal(h.p.positionMs,95000)
 const activity=h.api.leerEscuchaParaIntegraciones();assert.equal(activity.posicionMs,95000)
 h.emit({revision:4,suena:false,posicionMs:95000,arrancadoEn:null,actualizadoEn:105000})
 await h.advance(5000);assert.equal(h.p.positionMs,95000)
 assert.equal(h.api.leerEscuchaParaIntegraciones(),null);h.close()
})

test('un cambio remoto muestra canción y posición antes del refetch de la cola',async()=>{
 const h=fixture();await h.start()
 const next={...track,id:'nueva',videoId:'nueva',title:'Nueva'}
 h.emit({revision:4,track:next,posicionMs:45000,arrancadoEn:100000})
 assert.equal(h.p.tracks[h.p.index].id,'nueva')
 assert.equal(h.p.positionMs,45000)
 h.emit({revision:3,track,posicionMs:0});assert.equal(h.p.positionMs,45000)
 h.close()
})


test('al despertar el teléfono extrapola inmediatamente aunque el timer estuvo suspendido',async()=>{
 const h=fixture();h.remote({posicionMs:90000});await h.start()
 h.wake(7000)
 assert.equal(h.p.positionMs,97000)
 assert.equal(h.p.wantPlay,true)
 h.close()
})
